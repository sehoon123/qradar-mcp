...
"""
List Calculated Custom Properties Tool

Retrieves calculated custom event properties. These derive new values from other
properties via arithmetic expressions and can be referenced in AQL queries.
"""
...
class ListCalculatedPropertiesTool(MCPTool):
    """Tool for listing QRadar calculated custom event properties."""

    @property
    def name(self) -> str:
        return "list_calculated_properties"

    @property
    def description(self) -> str:
        return """List calculated custom event properties from QRadar with optional filtering, sorting, and pagination.

Calculated custom properties derive a new numeric value from other event
properties using an arithmetic expression. Their names can be referenced in AQL
queries. Use this to discover which calculated fields are available before
building an Ariel search.

Examples:
  - List all calculated properties: (no parameters)
  - Find by name: filter="name LIKE '%Duration%'"
  - Get first 100: limit=100, offset=0"""
    ...
    def input_schema(self) -> Dict[str, Any]:
        return (schema()
            .string("filter")
                .description('Optional AQL-style filter expression. Examples: "name LIKE \'%Duration%\'"')
            .string("fields")
    ...
    async def _execute_impl(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        response = await self.client.get(
            '/config/event_sources/custom_properties/calculated_properties',
    ...
    def _format_properties(self, properties: Any) -> str:
        if not properties:
            return "No calculated custom properties found"

        lines = [f"Found {len(properties)} calculated custom property(ies)\n", "=" * 80]
