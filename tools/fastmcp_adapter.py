...
def register_all_tools(mcp: FastMCP, toggle_manager, qradar_client) -> tuple:  # pylint: disable=too-many-locals,too-many-statements
    ...
    Import and register tool candidates using the adapter, filtering based on feature toggles.
    In read-only mode, QRadar-mutating tool classes are not imported as candidates.
    ...
    """
    # Import read-only and read-only-allowlisted tool classes.
    ...
    tools = [
        ...
        # Ariel tools (read-only and read-only-allowlisted)
        ...
        # Reference data tools - Maps (read-only legacy)
        ListReferenceMaps(),
        GetReferenceMap(),

        # Reference data tools - Tables (read-only)
        ListReferenceTables(),
        GetReferenceTable(),
        ...
        # QVM vulnerability management (14)
        ...
        # Composite read-only investigation workflows (2)
        GetOffenseInvestigationContextTool(),
        InvestigateOffenseEventsTool(),
    ]

    if not getattr(toggle_manager, "read_only_mode", False):
